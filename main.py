...
def check_payment_due(ledger, days_left):
    today = datetime.now().date()
    due_date = today + timedelta(days=days_left)
    upcoming_payments = [entry for entry in ledger if entry["date"] <= due_date]
...
while not b_is_exit:
    ...
    if func == "1":
        ...
        days_left = int(input("결제일 임박일 설정 (일): "))  # 결제일 임박일 설정을 받음
        check_payment_due(ledger, days_left)
    elif func == "?":
        print_help()
    elif func == "exit":
        b_is_exit = True
